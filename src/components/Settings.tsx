      ...
      <div className="p-5 space-y-6">
        <section>
          ...
          <div className="space-y-4">
            <Slider label="Default Volume" min={0} max={0.25} step={0.01} value={prefs.volume}
        ...
        <section>
          ...
          <div className="space-y-3">
            <Slider label="Detection Interval" min={1} max={30} step={1}
              value={prefs.detection_interval_secs}
              onChange={(v) => update("detection_interval_secs", v.toString())}
              valueFormatter={(v) => `Every ${v}s`} />
          </div>
          <div className="rounded-lg bg-surface-light border border-surface-lighter p-3 mt-3">
            <p className="text-xs text-text-secondary">
              Context detection reads your active window title and process name — no browser extension needed.
              It stays entirely on-device.
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            <Switch checked={prefs.auto_start} onChange={(v) => update("auto_start", v.toString())}
              label="Auto-start on login" description="Start TuneSoar when you log into your computer" />
            <Switch checked={prefs.minimize_to_tray} onChange={(v) => update("minimize_to_tray", v.toString())}
              label="Minimize to system tray" description="Close button minimizes instead of quitting" />
          </div>
        </section>
          ...
          <div className="rounded-lg bg-amber-600/10 border border-amber-600/30 p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs text-amber-300/80 leading-relaxed">
                ...
                <p>Binaural beats may trigger seizures in people with photosensitive epilepsy. Do not use if you have a history of seizures. Consult a doctor before use. Start at low volume (&lt;10%). Stop immediately if you feel discomfort.</p>
                <p className="mt-2 text-amber-400/60">{APP_NAME} enforces a max volume of 25%. Gamma (30-40 Hz) is disabled by default.</p>
              </div>
            </div>
          </div>
        </section>
